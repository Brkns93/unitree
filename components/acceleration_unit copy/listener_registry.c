...
int lr_register_listener(struct listener_registry_t* elr, const struct listener_t* listener){
    if(elr->listener_count == 0){
        elr->listeners = (struct listener_t*) malloc(sizeof(struct listener_t));
    }
    else{
        elr->listeners = (struct listener_t*) realloc(elr->listeners+elr->listener_count, sizeof(struct listener_t)*(elr->listener_count+1));
    }
